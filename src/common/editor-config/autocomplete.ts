    ...
    provideCompletionItems: (model: editor.ITextModel, position: Position) => {
      ...
      lines.forEach((line) => {
        if (line.includes(" '")) {
          const token = line
            .split(" '")
            .map((t) => t.replace(new RegExp(/\)/, "g"), ""))[1];
          if (token) {
...
export function hover(monaco: Monaco) {
  monaco.languages.registerHoverProvider("clarity", {
    provideHover: (model: editor.ITextModel, position: Position) => {
        ...
        ...new Set([
          ...[
            ...model
              ...
              .matchAll(hyphenWordPattern),
          ].flat(),
        ]),
      ];

      const token =
        (word?.word && wordsWithHyphens?.find((t) => t.includes(word.word))) ||
        word?.word;
          ...
          model
            ...
            .find((i) => {
              if (i.range.startLineNumber === position.lineNumber) {
                if (
                  position.column >= i.range.startColumn &&
                  position.column <= i.range.endColumn
                ) {
                  return true;
                }
              }
      ...
      const keywords = clarity.keywords.find(
